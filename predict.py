...
# Per-class confidence thresholds for auto-coding.
# A prediction is only auto-coded (needs_review = False) if its confidence
# is >= the threshold for its predicted class. Anything below is flagged
# for manual review, preferring manual review over a risky false auto-label.
CLASS_THRESHOLDS = {
    "1": 0.95,    # Lane 1: auto-code if 95%+ confident
    "2": 0.98,    # Lane 2: auto-code only if 98%+ confident
...
class PredictionDataset(Dataset):
    ...
    def __init__(self, df, path_prefix=""):
        # df: pandas dataFrame with filepath column
    ...
    def __getitem__(self, idx):
        ...
        gps_features = torch.tensor([
            float(row.get("lanes_forward", 1)),
...
def predict(input_csv, output_csv, checkpoint_path=None, threshold=0.90,
            batch_size=32, path_prefix="", device=None):
    # reads csv --> loads model --> predicts lanes --> saves results
    ...
    if "Ignore" in df.columns:
        ...
        print(f"[Predict] {ignore_mask.sum():,} rows skipped (Ignore=1)")
    else:
    ...
    print(f"[Predict] per-class thresholds: {CLASS_THRESHOLDS}")
    print(f"[Predict] fallback threshold (unlisted classes): {threshold:.0%}")
    ...
    # NOTE: column name is "Needs_review" (lowercase r) consistently below --
    # a previous version of this script initialized "Needs_review" but wrote
    # results into a differently-cased "Needs_Review", silently creating a
    # second column and leaving the original stuck at its initial value.
    df["Lane_AI"] = ""
...
def get_args():
    ...
    parser.add_argument("--threshold", type=float, default=0.90,
                         help="Fallback confidence threshold 0-1, used only for lane classes "
                              "not listed in CLASS_THRESHOLDS (default: 0.90)")
    parser.add_argument("--batch_size", type=int, default=32, help="Images per batch (default: 32)")
    parser.add_argument("--path_prefix", default="", help="Replace J:/ in paths with this prefix")
